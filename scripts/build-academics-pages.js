...
function normalizeSubjectName(str) {
  ...
  s = s.replace(
    /\b(VIII|VII|VI|V|IV|III|II|I)\b/gi,
    (m) => romanMap[m.toUpperCase()] || m,
  );
...
function resolveExistingSlug(slugMap, deptLower, subjectName) {
  ...
  const partial = list.find(
    (item) =>
      item.normalizedName.includes(target) ||
      target.includes(item.normalizedName),
  );
  return partial || null;
}
...
function buildMetaDescription(entry) {
  ...
  const deptName =
    ...
    DEPT_NAMES[entry.department_code] ||
  ...
  const reg = entry.regulation
    ...
    : "Regulation";
  return `${code} ${name} notes for Anna University ${deptName}. ${sem} ${reg}. Download PDFs, study notes, and revision materials.`
    .replace(/\s+/g, " ")
    .trim();
}
...
function buildStudyMaterialButtons(entry, folderSlug, subjectCode) {
  ...
  buttons.push(
    `    <a class="pyq-btn" href="${syllabusHref}" target="_blank" rel="noopener">View Syllabus</a>`,
  );

  if (code) {
    buttons.push(
      `    <a class="pyq-btn primary" href="${pyqHref}" target="_blank" rel="noopener">View Question Papers</a>`,
    );
  }
...
function buildHtml(entry, canonicalUrl, folderSlug) {
  ...
  const deptLabel =
    entry.department_name || DEPT_NAMES[deptCode] || deptCode || "Department";
  ...
  const regLabel = entry.regulation
    ? `Regulation ${entry.regulation}`
    : "Regulation";
...
if (!fs.existsSync(TEMPLATE_PATH)) {
  console.error(
    "academics-templates.json not found. Run scripts/generate-academics-templates.js first.",
  );
  process.exit(1);
}
...
entries.forEach((entry) => {
  ...
  const existing = resolveExistingSlug(
    existingSlugMap,
    deptLower,
    entry.subject_name || "",
  );
  ...
  const folderSlug =
    existing?.slug || (codeSlug ? `${codeSlug}-${nameSlug}` : nameSlug);
  if (!folderSlug) return;

  const canonicalUrl = `${BASE_URL}/anna-university-notes/${deptLower}/${folderSlug}/`;
  const html = buildHtml(
    { ...entry, subject_code: inferredCode || entry.subject_code },
    canonicalUrl,
    folderSlug,
  );
