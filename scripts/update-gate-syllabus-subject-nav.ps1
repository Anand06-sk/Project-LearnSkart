$root = Join-Path $PSScriptRoot "..\gate-syllabus"
$files = Get-ChildItem -Path $root -Recurse -Filter "index.html" | Where-Object { $_.DirectoryName -ne $root }
$newline = [Environment]::NewLine

foreach ($file in $files) {
    $slug = Split-Path -Path $file.DirectoryName -Leaf
    $gatePyqLink = "/gate/$slug/"
  ...
  <nav>
    <div class="nav-content">
      ...
      <div class="nav-right" id="navMenu">
        <div class="nav-links">
          <a href="/">Home</a>
          <a href="$gatePyqLink">GATE Previous Year Question Papers</a>
          <a href="$gateSyllabusLink" aria-current="page">GATE Syllabus</a>
